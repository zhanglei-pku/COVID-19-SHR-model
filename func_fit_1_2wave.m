function [DataTobeFitted,data_pre2,N_all,time_pre,...
    ...
    date_quarantined] = func_fit_diffWave(num_wave,inputarea)
...
[paraC,~] = fit_rate_infecetion(log(rate_qz(2:end)),SetPara0,...
    t_newconf_spl,S_lb,S_ub); 

% cure rate
SetPara0=[0.1 0.5 1];
[paraZ,~] = fit_rate_cure(log(rate_cure(2:end)),SetPara0,...
...
function [para,ypre]=fit_rate_infecetion(rate,SetPara0,t_newconf_spl,S_lb,S_ub)
tData = t_newconf_spl;
yData = rate;
...
% fit the cure rate
function [para,ypre] = fit_rate_cure(rate,SetPara0,t_newreco_spl,H_lb,H_ub)
tData = t_newreco_spl;
yData = rate;
