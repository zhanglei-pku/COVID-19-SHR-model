function [DataTobeFitted,data_pre2,N_all,time_pre,...
    ...
    date_quarantined,paraIRD_Optimal] = func_simulate_wave(num_wave,inputarea,para_lb,para_ub)
...
elseif num_wave == 2
    ind_start_confirmed = tableSD.ind_start2_confirmed(indSD);
    ind_end_confirmed   = tableSD.ind_end2_confirmed(indSD);
    ind_start_recovered = tableSD.ind_start2_recovered(indSD);
    ind_end_recovered  = tableSD.ind_end2_recovered(indSD);
elseif num_wave == 3
    ind_start_confirmed = tableSD.ind_start3_confirmed(indSD);
    ind_end_confirmed   = tableSD.ind_end3_confirmed(indSD);
    ind_start_recovered = tableSD.ind_start3_recovered(indSD);
    ind_end_recovered  = tableSD.ind_end3_recovered(indSD);
end
...
if ind_start_confirmed~=1
    cum_confirmed_fit = cum_confirmed_fit - Confirmed(ind_start_confirmed-1);
end
...
if ind_start_recovered~=1
    cum_recovered_fit = cum_recovered_fit - Recovered(ind_start_recovered-1);
end
...
[RSD0,ypre0] = fit_result_model(DataTobeFitted,Inputpara0(:,1),N_all,...
    t_gap_cumcure,t_gap_newcure);
...
function [para,ypre]=nlinfit_C_Simple(rate,SetPara0,t_newconf_spl,G_lb,G_ub)
tData = t_newconf_spl;
yData = rate;
